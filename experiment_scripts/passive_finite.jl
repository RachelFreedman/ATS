#= 
Run experiments using passive teacher selection on a finite-horizon, undiscounted POMDP.
to run:
    julia passive_finite.jl test_boolean num_items num_arms utility_granularity arm_granularity num_runs run_length state_index max_depth seed
=#
exp_name = "passive_finite_"
...
function O(s::State, a::Action, sp::State)
    ...
    if a.isBeta
        ...
        b = sp.b[rand(Random.seed!(params.seed), 1:end)]
        prob_of_pref = [Pr(o.p, sp, b) for o in P_obs]
        prob_of_query = Q
...
# solver = POMCPOW.POMCPOWSolver(max_depth=params.max_depth, rng=Random.seed!(params.seed), estimate_value=RolloutEstimator(Solvers.BestArmSolver(arm_actions, Random.seed!(params.seed))))
solver = POMCPOW.POMCPOWSolver(max_depth=params.max_depth, rng=Random.seed!(params.seed), estimate_value=RolloutEstimator(POMCPOW.BestArmSolver(actions(pomdp)[1:params.K], Random.seed!(params.seed))))
planner = solve(solver, pomdp);
log("solved POMDP using POMCPOW with max search depth "*string(params.max_depth)* " and rollouts simulated by BestArmSolver")
...
passive_R = Array{Float64}(undef, iters)
...
log("Passive R:\t(avg "*string(round(mean(passive_R),digits=0))*")\t"*string(passive_R))
log("Normalized R:\t(avg "*string(round(mean(passive_R./max_R),digits=2))*")\t"*string(passive_R./max_R))