#= 
Run experiments selecting all actions (arm pulls and teacher queries) uniformly at random.
to run:
    julia random.jl test_boolean num_items num_arms utility_granularity arm_granularity num_runs run_length state_index seed
=#
exp_name = "random_"
...
for iter in 1:iters
    ...
    mean_R[iter] = (mean([dot(initial_state.u, initial_state.d[i]) for i in 1:params.K])*steps)/(params.K/(params.K+params.M))
end
