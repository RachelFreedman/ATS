...
function search(pomcp::POMCPOWPlanner, tree::POMCPOWTree, info::Dict{Symbol,Any}=Dict{Symbol,Any}())
    ...
    while i < pomcp.solver.tree_queries
        ...
        if !POMDPs.isterminal(pomcp.problem, s)
            # RAF change -- allow undiscounted problems
            if discount(pomcp.problem) < 1.
                functional_discount = discount(pomcp.problem)
            else
                functional_discount = 0.999
            end
            max_depth = min(pomcp.solver.max_depth, ceil(Int, log(pomcp.solver.eps)/log(functional_discount)))
